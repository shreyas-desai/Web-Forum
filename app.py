...
def create_post():
    try:
        ...
        if 'parent_post' not in data:
            ...
            cursor.execute("SELECT 1 FROM posts WHERE id = ?",(parent_post_id,))
...
def get_post(post_id):
    try:
        ...
        cursor.execute('SELECT id FROM posts WHERE parent_post = ?', (post_id,))
        child_posts = []
...
def get_post_with_user():
    try:
        ...
        for post in post_data:
            ...
            cursor.execute('SELECT id FROM posts WHERE parent_post = ?', (post[0],))
            child_posts = []
            for i in cursor.fetchall():
                child_posts.append(i[0])
            print(child_posts)   
        ...
        for post in posts:
            print(post)
        row = cursor.execute('SELECT * from posts where user_id = ?',(user_id,))
        for i in row.fetchall():
            print(i)
        conn.commit()
        conn.close()
